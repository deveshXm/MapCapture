...
export const Input = React.forwardRef<HTMLInputElement, InputProps>(({ variant = "mixed", fancy = false, className, size = "md", ...props }, forwardedRef) => {
  const { input } = form();
  ...
  return <input ref={forwardedRef} className={input({ variant, fancy: fancy ? "input" : undefined, size, className })} {...props} />;
});
